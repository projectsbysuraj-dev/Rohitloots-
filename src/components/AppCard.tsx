  ...
  return (
    ...
    >
      ...
      <div className="p-3 sm:p-3.5 space-y-2 text-xs sm:text-sm">
        ...
        {/* Row 5: Rewards */}
        <div className="flex items-center justify-between text-slate-600 font-semibold border-t border-slate-100 pt-1.5">
          <span className="text-xs sm:text-sm font-bold">Rewards</span>
          <span className="font-black text-sky-600 text-sm sm:text-base">{formatDisplayAmount(app.rewardAmount, '350')}</span>
        </div>

        {/* Large 3D Sky Blue Pill Action Button */}
        <button
          onClick={() => setSelectedAppForClaim(app)}
          className="w-full py-2.5 px-3.5 rounded-full btn-3d-blue text-white font-black text-xs sm:text-sm flex items-center justify-center gap-2 cursor-pointer transition-all shadow-md active:scale-95 group mt-2"
        >
